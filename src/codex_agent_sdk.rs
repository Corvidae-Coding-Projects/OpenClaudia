...
impl CodexAgentSdk {
    ...
    pub async fn complete_turn(
        ...
        let mut stdin = child.stdin.take().ok_or_else(|| {
            CodexAgentSdkError::Spawn("child stdin was not available".to_string())
        })?;
        ...
        let stdout = child.stdout.take().ok_or_else(|| {
            CodexAgentSdkError::Spawn("child stdout was not available".to_string())
        })?;
        let stderr = child.stderr.take().ok_or_else(|| {
            CodexAgentSdkError::Spawn("child stderr was not available".to_string())
        })?;
        let stdout_task = tokio::spawn(read_bounded(stdout, MAX_CODEX_STDOUT_BYTES));
        let stderr_task = tokio::spawn(read_bounded(stderr, MAX_CODEX_STDERR_BYTES));
        let status = if let Ok(status) =
            tokio::time::timeout(CODEX_TURN_TIMEOUT, child.wait()).await
        {
            ...
            return Err(CodexAgentSdkError::Timeout(CODEX_TURN_TIMEOUT.as_secs()));
        };
...
async fn join_reader(
    task: tokio::task::JoinHandle<Result<Vec<u8>, CodexAgentSdkError>>,
