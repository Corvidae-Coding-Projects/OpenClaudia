...
- centralized `SessionState` module — Phases 0–2 (crosslink #510). `SessionState` groups identity, conversation, budget, and session UI data behind a clone-cheap `StateStore`; closure-based reads and writes prevent lock guards from crossing async boundaries, and granular `StateEvent`s are emitted after mutations release the lock. The TUI and legacy REPL now use the store as their single source of identity, messages, undo/redo, plan state, behavior mode, effort level, token estimate, UI flags, and additional working directories. Both frontends serialize through one backward-compatible `SessionDocument` with an embedded versioned canonical state, allowing TUI ↔ REPL resume without state loss while retaining legacy JSON support.
