...
- centralized `SessionState` module — Phases 0–3 (crosslink #510). `SessionState` groups identity, conversation, budget, session UI, permission, transcript, and IDE data behind a clone-cheap `StateStore`; closure-based reads and writes prevent lock guards from crossing async boundaries, and granular `StateEvent`s are emitted after mutations release the lock. The TUI and legacy REPL now use the store as their single source of identity, messages, undo/redo, plan state, behavior mode, effort level, token estimate, permission posture, transcript position, UI flags, and additional working directories. ACP IDE notifications update the same store, and bounded, escaped editor context now reaches the next provider prompt. Both frontends serialize through one backward-compatible `SessionDocument` with an embedded versioned canonical state, allowing TUI ↔ REPL resume without state loss while retaining legacy JSON support.
...
- Remove the Windows-only SecureDirectory dead-code warning (#1031)
- Eliminate background-job job.json generation conflicts in Linux CI (#1138)
- Eliminate background-job job.json generation conflicts in Linux CI (#1138)
...
- S-057: Replace lossy compaction with causal checkpoints (#1108)
...
- Finalize S-020 publication closure audit (#1032)
- Finalize S-020 publication closure audit (#1032)
- Implement and fully verify remediation slice S-020 (#1029)
- Stop closed issue titles from polluting the curated changelog (#1033)
- Finalize S-020 publication closure audit (#1032)
- Implement and fully verify remediation slice S-020 (#1029)
- Verify S-019 commit publication and draft PR state (#1027)
- S-017: fix deny precedence and approval scope (F-012, F-030, F-068) (#1007)
- Make permit consumption atomic with generation and exact-denial refresh (#1016)
- Reject over-broad persisted approval bounds and generation exhaustion (#1015)
- Web-search integration tests expect validation after mandatory classification denial (#1013)
- ACP sandbox cancellation can leave daemonized descendants alive (#1012)
- Windows lib build has unreachable and dead-code warnings (#1011)
- Web-search integration tests expect validation after mandatory classification denial (#1013)
- Tool caps count denied calls and race past configured limits (#1010)
- ACP sandbox cancellation can leave daemonized descendants alive (#1012)
- Windows lib build has unreachable and dead-code warnings (#1011)
- Tool caps count denied calls and race past configured limits (#1010)
- Permission receipt store directory sync is not portable to Windows (#1009)
